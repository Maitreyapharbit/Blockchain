    ...
    <Container>
      ...
      {lastBatch && (
        <StatusCard>
          ...
          {lastBatch.qrCode && (
            <QRCodeContainer>
              <QRCodeTitle>🔍 Unique QR Code Generated</QRCodeTitle>
              <QRCodeImage 
                src={lastBatch.qrCode.dataUrl} 
                alt="Batch QR Code"
                title="Scan this QR code to verify the batch"
              />
              <QRCodeInfo>
                <QRCodeText>
                  <strong>Batch URL:</strong> {lastBatch.qrCode.batchUrl}
                </QRCodeText>
                <QRCodeText>
                  <strong>QR Hash:</strong> {lastBatch.qrCode.hash.substring(0, 16)}...
                </QRCodeText>
                <DownloadButton 
                  onClick={() => {
                    const link = document.createElement('a');
                    link.href = lastBatch.qrCode.dataUrl;
                    link.download = `batch_${lastBatch.batchId}_qr.png`;
                    link.click();
                  }}
                >
                  📥 Download QR Code
